...
export const CONFIG = {
  ...
  kyoku: {
    ...
    // 手の点数分布（子のロン基準点, 重み）。実データ（天鳳・鳳凰卓赤あり）に整合:
    //   平均和了点 ~5,647（engine が親×1.5・ツモ分割を別途適用するので、ここは子基準で
    //   平均 ~4,900 → realized ~5,600）、満貫以上 ~15-20%、役満は稀。
    //   1000/2000/3900/5200/7700 と満貫(8000)/跳満(12000)/倍満(16000)/三倍満(24000)/役満(32000)。
    // レートに依らず点棒移動は同一（実麻雀どおり）。点5/点3の差は円レート・場代・祝儀のみ。
    handValues: [
      [1000, 14],
      [2000, 16],
      [3900, 18],
  ...
  // ---- 経済 ----
  // 場代（客1人・1半荘あたり）= プレイヤーの実売上。実勢（点5≈¥400/点3≈¥300）に近づけつつ、
  // 少数・高単価＋3半荘保証で利益中央値が ~¥42k(B帯中央) になるよう校正。
  // 点5/点3の差は「手の大きさ」ではなく、この場代差・円レート・祝儀＝実麻雀どおり。
  gameFeeYen: { BLUE: 540, GREEN: 350 } as Record<Rate, number>,
  // レート（1000点あたりの円）。点棒移動はレート非依存(blueHandMult撤去)なので、
  // 点5の資金変動の大きさ＝点5の唯一のリスク源はこのレート（実勢の点5¥50/点3¥30を参考に校正）。
