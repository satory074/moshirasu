...
A full playthrough is balanced for a **~5-minute session**: business hours run **17:00→24:30** (a ~7.5h evening rush, `openMin`/`hardCloseMin`), and since the game is turn-based the wall-clock ≈ Σ(decision-stops). An optimal-ish day is ~**72 decision-stops** / ~33 customers served / 点5中心で利益中央値 ~¥42k (`targetProfit`＝B帯中央; 詳細は Gotchas の `scoreRank` 帯). Fewer-but-higher-value customers (`baseArrivalChancePerTick` lower, `gameFeeYen` higher) keep the stop count down without starving profit. Session length is regression-guarded in `scripts/smoketest.ts` (the `[session]` block asserts avg stops stay in 45〜100).

Each 半荘 is simulated **局-by-局** (東1局〜南4局, 親番ローテ, 連荘/本場): every 局 moves **real 点棒** between the 4 seats (zero-sum, staff included) via a **real-mahjong payment structure** (親 ×1.5, ツモ split 子/子/親, 本場 ±300/±100×3 — see `resolveKyoku`), and final placement is decided by accumulated 点棒 — not a one-shot roll. 点5(BLUE) hands swing harder (`blueHandMult`) and pay a higher yen rate (`rateYenPer1000.BLUE`), so a thin-stacked customer 飛ぶ (busts) a few times across a 点5-heavy day. Up to **8 tables** (`CONFIG.maxTables`).
...
- **局シミュ定数は `CONFIG.kyoku` に集約**: `kyokuMin`/`ryuukyokuProb`/`tsumoProb`/`winSkillW`/`winLuckW`/`dealerWinMult`/`handValues`/`blueHandMult`/`maxKyokuPerHanchan`/`maxHonba`/`seatedWaitMult`。半荘長・飛び頻度・1日の半荘数（=売上）はここで決まる。`handValues`/`blueHandMult`(現2.4) を上げると点5の飛びが増えるが反動でブレも増える。点5の旨味は `gameFeeYen.BLUE`(現¥950・点3¥480) が主リターン源、`rateYenPer1000.BLUE`(現¥110) と `reputation.bustHit`(現2.5) が飛びの怖さを調整する。**スコア評価 `scoreRank(利益)` は実測分布に整合した絶対帯**（S≥¥65k/A≥¥52k/B≥¥34k/C≥¥20k/D<¥20k。`targetProfit`=¥42k は B 中央の表示アンカー）。多シード実測で点5中心の利益中央値 ~¥42k＝B になるよう校正（旧 `targetProfit`×係数方式は当たりシード寄りで不振日がD落ちしていた）。smoketest が `[blue-aggressive]`(飛び≥3)・`[blue-vs-green]`(点5中心>点3一辺倒×1.3)・`[score-band]`(平均がB帯) を回帰ガード。`Stats.blueHanchanPlayed` は点5中心プレイ判定（称号「点5の鬼」）用。
