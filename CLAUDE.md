...
A full playthrough is balanced for a **~5-minute session**: business hours run **17:00→24:30** (a ~7.5h evening rush, `openMin`/`hardCloseMin`), and since the game is turn-based the wall-clock ≈ Σ(decision-stops). An optimal-ish day is ~**72 decision-stops** / ~33 customers served / ~¥50k profit (`targetProfit`). Fewer-but-higher-value customers (`baseArrivalChancePerTick` lower, `gameFeeYen` higher) keep the stop count down without starving profit. Session length is regression-guarded in `scripts/smoketest.ts` (the `[session]` block asserts avg stops stay in 45〜100).
